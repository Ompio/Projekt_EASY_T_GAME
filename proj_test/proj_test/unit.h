...
public:
	...
	void attack_M(coords x, std::vector<std::shared_ptr<unit>>& loadedEntities);
	void deal_damage(std::shared_ptr<unit>& enemy);
	void modify_Hp(int hp_value);
	bool check_living(std::vector<std::vector<int> >& map, std::vector<std::shared_ptr<unit>>& loadedEntities);
